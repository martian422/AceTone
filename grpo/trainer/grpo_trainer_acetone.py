...
def pref_score(scores: torch.Tensor) -> torch.Tensor:
    ...
    # If the scores are already binary, usually the format reward, return them as is.
    if torch.all((scores == 0) | (scores == 1)):
        return scores.view(-1)
    
    # Otherwise, normalize the scores to [0, 1] range.
...
class AcetoneGRPOTrainer(Trainer):
    """
    ...
    def __init__(
        ...
        if isinstance(model, str):
            ...
            elif "acetone" in model_id:
                model_config = AceToneConfig.from_pretrained(model, **model_init_kwargs)
                model_config.model_type = "acetone"
                model_config.mm_vocab_size = 256
                print("Loading custom AceTone model...")
                model = AceToneVLM.from_pretrained(model, config = model_config)
            else:
        ...
        if is_deepspeed_zero3_enabled():
            if "Qwen2-VL" in model_id:
                ...
                self.ref_model = AceToneVLM.from_pretrained(model_id, config = model_config)
            else:
        ...
        if processing_class is None:
            if "Qwen2-VL" in model_id or "Qwen2.5-VL" in model_id or "acetone" in model_id:
                processing_class = AutoProcessor.from_pretrained(model_id)
                pad_token_id = processing_class.tokenizer.pad_token_id
                processing_class.pad_token_id = pad_token_id
                processing_class.eos_token_id = processing_class.tokenizer.eos_token_id
                if "Qwen" in model_id or "Qwen2.5-VL" or "acetone" in model_id:
        ...
        self.use_pref = args.use_pref
        if self.use_pref:
            print("Using Pref-GRPO design.")
    ...
    def _generate_and_score_completions(
        ...
        for ex in inputs:
            msg = [
                ...
                {"role": "user", "content":[
                    {"type":"image","image": ex['ref_image_path']},
                    {"type":"image","image": ex['ori_image_path']},
        ...
        # for AceTone, the <SoT> and <EoT> are introduced as special tokens for proper wrap-up of LUTs.
        ...
        for i, reward_func in enumerate(self.reward_funcs):
            ...
            self._metrics[f"{reward_func_name}"].append(mean_rewards)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # remove the average over the sequence length?
        # loss = ((per_token_loss * completion_mask).sum(dim=1) / 72).mean()
